...
| 操作 | 语法 | 参数 | 返回值 | 适用场景 |
|------|------|------|--------|----------|
| `del` | `del list[index]` | 索引或切片 | 无 | 需要直接删除不关心返回值时 |
| `remove()` | `list.remove(value)` | 要删除的值 | 无 | 按值删除第一个匹配项时 |
| `pop()` | `list.pop([index])` | 可选索引(默认-1) | 被删除的元素 | 需要获取被删除元素时 |
| `clear()` | `list.clear()` | 无 | 无 | 清空整个列表保留空列表对象时 |
