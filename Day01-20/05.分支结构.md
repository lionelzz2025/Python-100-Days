...
### deepseek补充
Python 3.10+ 引入的 `match-case` 结构（结构化模式匹配）确实支持范围匹配。具体来说：

### 基本范围匹配：
可以使用比较运算符（如 `<`, `>`, `<=`, `>=`）结合 `case` 语句：
...
print(assess_score(85))  # 输出: B
```

注意：
1. 范围检查需要使用 `if` （`case ... if ...`）
2. 匹配是按顺序进行的，所以应该从最具体的情况开始
3. `_` 是通配符，匹配任何值

相较简单的范围检查，`match-case` 更适合有多个复杂条件分支的场景。简单范围检查用 `if-elif-else` 可能更直观。
